...
import oauth2
from database import User,User_token
...
def login(payload: schemas.LoginUserSchema, response: Response, Authorize: AuthJWT = Depends()):
    ...
    #delete token
    User_token.find_one_and_delete({'email': payload.email.lower()})
    
    new_user_token = User_token.insert_one({'email':payload.email.lower(),'user':user,'token':access_token,'expires_at':datetime.utcnow().timestamp()+ ACCESS_TOKEN_EXPIRES_IN*60})
